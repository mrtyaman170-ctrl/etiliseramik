...
function bind(){
  ...
  document.querySelectorAll(".fault-participant-check").forEach(check=>check.onchange=()=>{
    ...
    if(!fault.participants.includes(fault.assignedTo))fault.assignedTo=fault.participants[0];
    save();
    render();
  });
  ...
  document.querySelectorAll(".status-sel").forEach(x=>x.onchange=()=>{
    ...
    if(x.value==="progress"&&(!fault.assignedTo||fault.assignedTo==="Atama Bekliyor")){
      fault.assignedTo=deterministicPerson(availableMaintenanceForFault(fault),fault.id,19)||"Atama Bekliyor";
    }
    if(x.value==="done"){
  ...
  document.querySelectorAll(".personnel-sel").forEach(x=>x.onchange=()=>{
    ...
    if(fault&&canRedirectFault(fault)){fault.assignedTo=x.value;save();render()}
  });
  ...
  if(addPm)addPm.onclick=()=>{s.plannedSelectedDate=dateOnly(new Date());s.plannedModal=true;render()};
  ...
  document.querySelectorAll("[data-calendar-date]").forEach(day=>day.ondblclick=()=>{
    if(!permissions().editPlanned)return;
    s.plannedSelectedDate=day.dataset.calendarDate;s.plannedModal=true;render();
  });
  document.querySelectorAll(".planned-close").forEach(x=>x.onclick=()=>{s.plannedModal=false;render()});
  const pmBg=document.querySelector(".planned-modal-bg");
  if(pmBg)pmBg.onclick=e=>{if(e.target===pmBg){s.plannedModal=false;render()}};
  ...
  if(pmFactory)pmFactory.onchange=()=>{
    ...
    const departments=catalogDepartments(pmFactory.value,pmLine.value);
    pmDepartment.innerHTML=opts(departments,"Bölüm seçiniz",departments[0]||"");
  ...
  if(pmForm)pmForm.onsubmit=e=>{
    ...
    savePlanned();s.plannedModal=false;render();
  };
  document.querySelectorAll(".pm-status").forEach(x=>x.onchange=()=>{
    const item=s.plannedMaintenances.find(y=>String(y.id)===String(x.dataset.id));
    if(item){item.status=x.value;savePlanned();render()}
  });
