...
const SHIFT_TEMPLATE_COLORS=["5B9BD5","70AD47","ED7D31","A5A5A5","FFC000","4472C4","A9D18E","F4B183"];
function shiftExportSafeText(value){
  return String(value||"").trim().replace(/[^\p{L}\p{N}]+/gu,"_").replace(/^_+|_+$/g,"")||"Vardiya";
}
function exportShiftTemplateFile(factory,team,monthValue){
  if(!window.XLSX)throw new Error("Excel çıktı bileşeni yüklenemedi. Sayfayı yenileyip tekrar deneyin.");
  ...
  const schedule=buildTeamMonthSchedule(factory,team,selectedMonth);
  const totalColumns=1+schedule.dates.length*3;
  const empty=()=>Array(totalColumns).fill("");
  const title=empty();title[0]="VARDİYALI ÇALIŞMA ÇİZELGESİ";
  const details=empty();details[0]="BÖLÜM";details[1]=`${factory} · ${team}`;details[5]="AY";details[6]=selectedMonth.toLocaleDateString("tr-TR",{month:"long"}).toLocaleUpperCase("tr-TR");details[9]="YIL";details[10]=selectedMonth.getFullYear();
  const weekdays=["GÜN"],days=[""],shifts=["PERSONEL ADI"];
  schedule.dates.forEach(date=>{
    weekdays.push(date.toLocaleDateString("tr-TR",{weekday:"long"}).toLocaleUpperCase("tr-TR"),"","");
    days.push(date.getDate(),"","");
    shifts.push("24 / 8","8 / 16","16 / 24");
  });
  const records=schedule.rows.map(row=>{
    const values=[row.name];
    row.days.forEach(day=>{
      if(day.shift===SHIFT_OFF)values.push("İZİN","","");
      else SHIFT_LABELS.forEach(shift=>values.push(day.shift===shift?"●":""));
    });
    return values;
  });
  const sheet=XLSX.utils.aoa_to_sheet([title,empty(),details,weekdays,days,shifts,...records]);
  const merges=[{s:{r:0,c:0},e:{r:0,c:totalColumns-1}}];
  schedule.dates.forEach((_,index)=>{
    const start=1+index*3;
    merges.push({s:{r:3,c:start},e:{r:3,c:start+2}});
    merges.push({s:{r:4,c:start},e:{r:4,c:start+2}});
  });
  sheet["!merges"]=merges;
  sheet["!cols"]=[{wch:26},...Array.from({length:totalColumns-1},()=>({wch:10}))];
  sheet["!rows"]=[{hpt:26},{hpt:9},{hpt:19},{hpt:20},{hpt:19},{hpt:23}];
  const border={top:{style:"thin",color:{rgb:"B8C4CE"}},bottom:{style:"thin",color:{rgb:"B8C4CE"}},left:{style:"thin",color:{rgb:"B8C4CE"}},right:{style:"thin",color:{rgb:"B8C4CE"}}};
  const setStyle=(row,column,style)=>{
    const address=XLSX.utils.encode_cell({r:row,c:column});
    if(!sheet[address])sheet[address]={t:"s",v:""};
    sheet[address].s=style;
  };
  setStyle(0,0,{font:{bold:true,sz:16,color:{rgb:"FFFFFF"}},fill:{patternType:"solid",fgColor:{rgb:"26333F"}},alignment:{horizontal:"center",vertical:"center"}});
  for(let column=0;column<totalColumns;column++){
    setStyle(3,column,{font:{bold:true,sz:9,color:{rgb:"FFFFFF"}},fill:{patternType:"solid",fgColor:{rgb:"3D5265"}},alignment:{horizontal:"center",vertical:"center",wrapText:true},border});
    setStyle(4,column,{font:{bold:true,sz:9,color:{rgb:"26333F"}},fill:{patternType:"solid",fgColor:{rgb:"DCE6EE"}},alignment:{horizontal:"center",vertical:"center"},border});
    setStyle(5,column,{font:{bold:true,sz:9,color:{rgb:"FFFFFF"}},fill:{patternType:"solid",fgColor:{rgb:"2F75B5"}},alignment:{horizontal:"center",vertical:"center"},border});
  }
  [0,1,5,6,9,10].forEach(column=>setStyle(2,column,{font:{bold:true,sz:9,color:{rgb:"26333F"}},fill:{patternType:"solid",fgColor:{rgb:"EAF2F8"}},alignment:{horizontal:column===1||column===6?"left":"center",vertical:"center"},border}));
  schedule.rows.forEach((row,rowIndex)=>{
    const excelRow=6+rowIndex;
    setStyle(excelRow,0,{font:{bold:true,sz:9,color:{rgb:"26333F"}},fill:{patternType:"solid",fgColor:{rgb:"F6F8FA"}},alignment:{vertical:"center"},border});
    row.days.forEach((day,dayIndex)=>{
      const start=1+dayIndex*3;
      const color=SHIFT_TEMPLATE_COLORS[rowIndex%SHIFT_TEMPLATE_COLORS.length];
      const activeIndexes=day.shift===SHIFT_OFF?[0,1,2]:[Math.max(0,SHIFT_LABELS.indexOf(day.shift))];
      for(let offset=0;offset<3;offset++){
        const active=activeIndexes.includes(offset);
        setStyle(excelRow,start+offset,{font:{bold:active,sz:10,color:{rgb:active?"FFFFFF":"52616C"}},fill:{patternType:"solid",fgColor:{rgb:active?color:"FFFFFF"}},alignment:{horizontal:"center",vertical:"center"},border});
      }
    });
  });
  const workbook=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook,sheet,"Vardiya Çizelgesi");
  const filename=`ETILISMART_${shiftExportSafeText(factory)}_${shiftExportSafeText(team)}_${selectedMonth.getFullYear()}_${String(selectedMonth.getMonth()+1).padStart(2,"0")}.xlsx`;
...
function shiftSchedulePage(){
  ...
  <section class="shift-filter-card team-only">
    ...
    <button type="button" class="secondary shift-export-button" id="exportShiftTemplate">⇩ Excel Şablonu İndir</button>
  </section>
