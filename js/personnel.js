...
function enforceSystemAccountScopes(users){
  // Önceki sürümden tarayıcıda kalan kullanıcı kaydı, sabit sistem
  // hesaplarının fabrika/rol sınırını genişletemez.
  Object.entries(SYSTEM_ACCOUNT_SCOPES).forEach(([id,scope])=>{
    if(!users[id])return;
    users[id]={...users[id],...scope,factories:[...scope.factories]};
  });
  return users;
}
...
function loadAppUsers(){
  ...
  const merged=enforceSystemAccountScopes(removeLegacyUsers({
