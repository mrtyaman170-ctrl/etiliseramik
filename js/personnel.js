...
const ROLES=["Operatör","Bakım Personeli","Bölüm Formeni","Bölüm Yöneticisi","Üretim Müdürü","Bakım Formeni","Bakım Müdürü","Genel Müdür","Yazılımcı"];
const TYPES=["Elektrik","Mekanik","Otomasyon","Pnömatik","Hidrolik","Diğer"];

const AUTH_VERSION="pc-v4";
...
const DEFAULT_USERS=removeLegacyUsers({...DEMO_USERS,...FACTORY_DEPARTMENT_FOREMEN,...PRODUCTION_MANAGER_USERS,...DEVELOPER_USERS});
function loadAppUsers(){
  const saved=safeJson(storageGet(localStorage,USER_STORE_KEY,"null"),null);
  if(saved&&typeof saved==="object"){
    const merged=removeLegacyUsers({...JSON.parse(JSON.stringify(DEMO_USERS)),...saved,...JSON.parse(JSON.stringify(FACTORY_DEPARTMENT_FOREMEN)),...JSON.parse(JSON.stringify(PRODUCTION_MANAGER_USERS)),...JSON.parse(JSON.stringify(DEVELOPER_USERS))});
    storageSet(localStorage,USER_STORE_KEY,JSON.stringify(merged));
    return merged;
  }
  storageSet(localStorage,USER_STORE_KEY,JSON.stringify(DEFAULT_USERS));
...
function faultParticipants(fault){
  return [...new Set(Array.isArray(fault?.participants)?fault.participants.filter(Boolean):[])];
}
...
function canClaimFault(fault){
  ...
  return !assigned||["Atama Bekliyor","Bakım Ekibi"].includes(assigned)||assigned===s.user?.name;
}
...
function maintenanceLogParticipants(log){
  return [...new Set(Array.isArray(log?.participants)?log.participants.filter(Boolean):[])];
}
...
function deleteMaintenancePersonnelAccount(id){
  ...
  s.faults.forEach(f=>{
    ...
    if(f.assignedTo===account.name)f.assignedTo=remaining[0]||"";
  });
...
const ROLE_PERMISSIONS={
  ...
  "Bakım Müdürü":{dashboard:true,newFault:false,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,personnel:true,materials:true,workRequests:true,editPlanned:true,editStatus:true,manageShifts:true,redirectFaults:true,manageAllPersonnel:true,viewPerformance:true,allFactories:true,manageFaultMaterials:true,manageRequests:true,createDirectWorkOrder:true,dailyChecks:true,manageDailyChecks:true,manageMachines:true,manageDailyControlCatalog:true},
  "Elektrik Bakım Formeni":{dashboard:true,newFault:false,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,personnel:true,materials:true,workRequests:true,editPlanned:true,editStatus:true,manageShifts:true,redirectFaults:true,manageOwnTeam:true,viewPerformance:true,manageFaultMaterials:true,manageRequests:true,createDirectWorkOrder:true,dailyChecks:true,manageDailyChecks:true,manageMachines:true,manageDailyControlCatalog:true},
  "Mekanik Bakım Formeni":{dashboard:true,newFault:false,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,personnel:true,materials:true,workRequests:true,editPlanned:true,editStatus:true,manageShifts:true,redirectFaults:true,manageOwnTeam:true,viewPerformance:true,manageFaultMaterials:true,manageRequests:true,createDirectWorkOrder:true,dailyChecks:true,manageDailyChecks:true,manageMachines:true,manageDailyControlCatalog:true},
  "Bakım Formeni":{dashboard:true,newFault:false,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,personnel:true,materials:true,workRequests:true,editPlanned:true,editStatus:true,manageShifts:true,redirectFaults:true,manageOwnTeam:true,viewPerformance:true,manageFaultMaterials:true,manageRequests:true,createDirectWorkOrder:true,dailyChecks:true,manageDailyChecks:true,manageMachines:true,manageDailyControlCatalog:true},
  "Bölüm Formeni":{dashboard:true,newFault:true,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,materials:true,workRequests:true,createRequest:true,editPlanned:true,editStatus:false,departmentOnly:true},
  "Üretim Müdürü":{dashboard:true,newFault:true,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,materials:true,workRequests:true,createRequest:true,editPlanned:true,editStatus:false,allDepartments:true,allDepartmentForemanRights:true},
  "Bakım Personeli":{dashboard:true,newFault:false,faults:true,report:false,layout:true,planned:true,shiftSchedule:true,materials:true,workRequests:true,editPlanned:false,editStatus:true,manageFaultMaterials:true,updateAssignedWorkOrders:true,viewAllShiftFactories:true,dailyChecks:true,completeDailyChecks:true},
