...
function loadAppUsers(){
  ...
  storageSet(localStorage,USER_STORE_KEY,JSON.stringify(DEMO_USERS));
...
function faultParticipants(fault){
  const list=Array.isArray(fault?.participants)?fault.participants.filter(Boolean):[];
  if(fault?.assignedTo&&!list.includes(fault.assignedTo))list.unshift(fault.assignedTo);
  return [...new Set(list)];
}
...
function canAddMaintenanceLog(){
  return ["Bakım Personeli","Elektrik Bakım Formeni","Mekanik Bakım Formeni","Bakım Formeni","Bakım Müdürü"].includes(s.user?.role);
}
...
function canHandoverFault(fault){
  ...
  if(canRedirectFault(fault))return true;
  return s.user?.role==="Bakım Personeli"&&faultParticipants(fault).includes(s.user?.name)&&
...
const ROLE_PERMISSIONS={
  ...
  "Fabrika Müdürü":{dashboard:true,newFault:false,faults:true,report:true,layout:true,planned:true,shiftSchedule:false,personnel:true,materials:true,workRequests:true,editPlanned:false,editStatus:false,viewPerformance:true,dailyChecks:true},
  ...
  "Bakım Formeni":{dashboard:true,newFault:false,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,personnel:true,materials:true,workRequests:true,editPlanned:true,editStatus:true,manageShifts:true,redirectFaults:true,manageOwnTeam:true,viewPerformance:true,manageFaultMaterials:true,manageRequests:true,createDirectWorkOrder:true,dailyChecks:true,manageDailyChecks:true,manageMachines:true,manageDailyControlCatalog:true},
  "Bölüm Formeni":{dashboard:true,newFault:true,faults:true,report:true,layout:true,planned:true,shiftSchedule:true,materials:true,workRequests:true,createRequest:true,editPlanned:false,editStatus:false,departmentOnly:true},
