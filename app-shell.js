...
function table(items,editable=false){
  ...
  return `<div class="card table-wrap"><table>
    ...
    <tbody>${items.map(f=>`<tr class="fault-click-row" data-fault-detail-id="${f.id}" title="Arıza detaylarını görüntülemek için tıklayın">
      ...
      ${editable?`<td data-label="İşlem"><select class="sel status-sel" data-id="${f.id}">
...
function ensureFaultPersonnel(){
  ...
  s.faults.forEach((fault,index)=>{
    ...
    if(!Object.prototype.hasOwnProperty.call(fault,"assignedTo")||!fault.assignedTo||fault.assignedTo==="Atama Bekliyor"){
      const people=availableMaintenanceForFault(fault);
      fault.assignedTo=deterministicPerson(people,fault.id,index+7)||deterministicPerson(maintenanceOptionsForFault(fault),fault.id,index+17)||"Bakım Ekibi";
      changed=true;
    }
  });
  if(changed)save();
